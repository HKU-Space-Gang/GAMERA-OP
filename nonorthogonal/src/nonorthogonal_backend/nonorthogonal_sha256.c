#include "nonorthogonal_sha256.h"

#include <string.h>

static uint32_t rotate_right(uint32_t value, unsigned count) {
  return (value >> count) | (value << (32U - count));
}

static uint32_t load_be32(const unsigned char *value) {
  return ((uint32_t)value[0] << 24U) | ((uint32_t)value[1] << 16U) |
         ((uint32_t)value[2] << 8U) | (uint32_t)value[3];
}

static void store_be32(unsigned char *output, uint32_t value) {
  output[0] = (unsigned char)(value >> 24U);
  output[1] = (unsigned char)(value >> 16U);
  output[2] = (unsigned char)(value >> 8U);
  output[3] = (unsigned char)value;
}

static void transform(gamera_no_sha256 *context,
                      const unsigned char block[64]) {
  static const uint32_t constant[64] = {
      0x428a2f98U, 0x71374491U, 0xb5c0fbcfU, 0xe9b5dba5U,
      0x3956c25bU, 0x59f111f1U, 0x923f82a4U, 0xab1c5ed5U,
      0xd807aa98U, 0x12835b01U, 0x243185beU, 0x550c7dc3U,
      0x72be5d74U, 0x80deb1feU, 0x9bdc06a7U, 0xc19bf174U,
      0xe49b69c1U, 0xefbe4786U, 0x0fc19dc6U, 0x240ca1ccU,
      0x2de92c6fU, 0x4a7484aaU, 0x5cb0a9dcU, 0x76f988daU,
      0x983e5152U, 0xa831c66dU, 0xb00327c8U, 0xbf597fc7U,
      0xc6e00bf3U, 0xd5a79147U, 0x06ca6351U, 0x14292967U,
      0x27b70a85U, 0x2e1b2138U, 0x4d2c6dfcU, 0x53380d13U,
      0x650a7354U, 0x766a0abbU, 0x81c2c92eU, 0x92722c85U,
      0xa2bfe8a1U, 0xa81a664bU, 0xc24b8b70U, 0xc76c51a3U,
      0xd192e819U, 0xd6990624U, 0xf40e3585U, 0x106aa070U,
      0x19a4c116U, 0x1e376c08U, 0x2748774cU, 0x34b0bcb5U,
      0x391c0cb3U, 0x4ed8aa4aU, 0x5b9cca4fU, 0x682e6ff3U,
      0x748f82eeU, 0x78a5636fU, 0x84c87814U, 0x8cc70208U,
      0x90befffaU, 0xa4506cebU, 0xbef9a3f7U, 0xc67178f2U};
  uint32_t word[64];
  for (size_t index = 0U; index < 16U; ++index) {
    word[index] = load_be32(block + 4U * index);
  }
  for (size_t index = 16U; index < 64U; ++index) {
    const uint32_t s0 = rotate_right(word[index - 15U], 7U) ^
                        rotate_right(word[index - 15U], 18U) ^
                        (word[index - 15U] >> 3U);
    const uint32_t s1 = rotate_right(word[index - 2U], 17U) ^
                        rotate_right(word[index - 2U], 19U) ^
                        (word[index - 2U] >> 10U);
    word[index] = word[index - 16U] + s0 + word[index - 7U] + s1;
  }
  uint32_t a = context->state[0];
  uint32_t b = context->state[1];
  uint32_t c = context->state[2];
  uint32_t d = context->state[3];
  uint32_t e = context->state[4];
  uint32_t f = context->state[5];
  uint32_t g = context->state[6];
  uint32_t h = context->state[7];
  for (size_t index = 0U; index < 64U; ++index) {
    const uint32_t s1 =
        rotate_right(e, 6U) ^ rotate_right(e, 11U) ^ rotate_right(e, 25U);
    const uint32_t choose = (e & f) ^ ((~e) & g);
    const uint32_t first = h + s1 + choose + constant[index] + word[index];
    const uint32_t s0 =
        rotate_right(a, 2U) ^ rotate_right(a, 13U) ^ rotate_right(a, 22U);
    const uint32_t majority = (a & b) ^ (a & c) ^ (b & c);
    const uint32_t second = s0 + majority;
    h = g;
    g = f;
    f = e;
    e = d + first;
    d = c;
    c = b;
    b = a;
    a = first + second;
  }
  context->state[0] += a;
  context->state[1] += b;
  context->state[2] += c;
  context->state[3] += d;
  context->state[4] += e;
  context->state[5] += f;
  context->state[6] += g;
  context->state[7] += h;
}

void gamera_no_sha256_init(gamera_no_sha256 *context) {
  *context = (gamera_no_sha256){{
      0x6a09e667U, 0xbb67ae85U, 0x3c6ef372U, 0xa54ff53aU,
      0x510e527fU, 0x9b05688cU, 0x1f83d9abU, 0x5be0cd19U},
      0U, {0U}, 0U};
}

void gamera_no_sha256_update(gamera_no_sha256 *context, const void *data,
                             size_t bytes) {
  const unsigned char *input = data;
  context->bytes += bytes;
  while (bytes > 0U) {
    const size_t available = 64U - context->used;
    const size_t copied = bytes < available ? bytes : available;
    memcpy(context->block + context->used, input, copied);
    context->used += copied;
    input += copied;
    bytes -= copied;
    if (context->used == 64U) {
      transform(context, context->block);
      context->used = 0U;
    }
  }
}

void gamera_no_sha256_final(gamera_no_sha256 *context,
                            unsigned char digest[32]) {
  const uint64_t bits = context->bytes * 8U;
  context->block[context->used++] = 0x80U;
  if (context->used > 56U) {
    memset(context->block + context->used, 0, 64U - context->used);
    transform(context, context->block);
    context->used = 0U;
  }
  memset(context->block + context->used, 0, 56U - context->used);
  for (size_t byte = 0U; byte < 8U; ++byte) {
    context->block[63U - byte] = (unsigned char)(bits >> (8U * byte));
  }
  transform(context, context->block);
  for (size_t index = 0U; index < 8U; ++index) {
    store_be32(digest + 4U * index, context->state[index]);
  }
  memset(context, 0, sizeof(*context));
}
