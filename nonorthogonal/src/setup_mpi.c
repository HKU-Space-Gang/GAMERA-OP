...
static int setup_cart_comm(int rank) {
  ...
  const int ranks_per_patch =
      config.proc_dims[0] * config.proc_dims[1] * config.proc_dims[2];
  if (patch_count <= 0 || ranks_per_patch <= 0 ||
      size != patch_count * ranks_per_patch) {
    log_error("Invalid overset MPI topology: world=%d patches=%d "
              "ranks_per_patch=%d", size, patch_count, ranks_per_patch);
    return -1;
  }
  patch_size = ranks_per_patch;
  patch_id = rank / patch_size;
  patch_rank = rank % patch_size;

  MPI_Comm patch_comm = MPI_COMM_NULL;
  int status = MPI_Comm_split(MPI_COMM_WORLD, patch_id, patch_rank,
                              &patch_comm);
  if (status != MPI_SUCCESS) {
    log_error("Failed to split patch communicator");
    return status;
  }

  int reorder = 0;
  status = MPI_Cart_create(patch_comm, 3, config.proc_dims, periods,
                           reorder, &comm_cart);
  MPI_Comm_free(&patch_comm);
  if (status != 0) {
  ...
  int cart_rank = -1;
  status = MPI_Comm_rank(comm_cart, &cart_rank);
  if (status != MPI_SUCCESS) {
    log_error("Failed to get patch Cartesian rank.");
    return status;
  }
  status = MPI_Cart_coords(comm_cart, cart_rank, 3, proc_coords);
  if (status != 0) {
